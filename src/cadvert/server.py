...
async def chat(
    ...
    system_msg = (
        ...
        "You have been given a Tier-0 summary of a CAD part — a compact document "
        "with exact geometry extracted from the B-REP model.\n\n"
        "Rules:\n"
        "- Answer questions using the exact numerical values in the summary.\n"
        "- When you need more detail (specific face geometry, edge connectivity, "
        "exact distance measurements), USE THE AVAILABLE TOOLS to retrieve it.\n"
        "- Reference faces as F12, edges as E5, features by their ID (hole_1, etc.).\n"
        "- If a value is not in the summary and you cannot get it via a tool, say so.\n"
        "- For mesh-format files (STL/OBJ), note that exact geometry is unavailable.\n"
        "- Be concise but precise. Units are in the document header.\n\n"
        f"<PART_SUMMARY>\n{tier0}\n</PART_SUMMARY>"
    )
